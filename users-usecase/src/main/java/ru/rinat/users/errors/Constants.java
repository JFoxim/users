package ru.rinat.users.errors;

public final class Constants {

    public static final String EXCEPTION_WRONG_PARAMETER = "EXCEPTION_WRONG_PARAMETER";

    public static final String COMMON_EXCEPTION = "COMMON_EXCEPTION";

    public static final String USER_NOT_FOUND_EXCEPTION = "USER_NOT_FOUND_EXCEPTION";

    public static final String NEWS_NOT_FOUND_EXCEPTION = "NEWS_NOT_FOUND_EXCEPTION";

    public static final String CONTACT_NOT_FOUND_EXCEPTION = "CONTACT_NOT_FOUND_EXCEPTION";

    public static final String DEFAULT_EXCEPTION_MESSAGE = "DEFAULT_EXCEPTION_MESSAGE";
}
